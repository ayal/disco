...
var gen = _.debounce(function(okgo) {
	...
	for (var h =  0; h < 3; h++) {
		...
		for (var i = 0; i < 16; ++i) {
		    ...
		    // R
		    if (i % 2 === 0) {
			makesound(bufferList[0], rnd(1,2)/3, 0.02, time(i+4), time(i));
		    }

		    // snare
		    if ([4,7,9,12,15].indexOf(i) !== -1 && [0,1].indexOf(j) !== -1) {
			makesound(bufferList[1], rnd(1,2)/3, 0.05, time(i+1), time(i));
		    }

		    if ([4,7,9,14].indexOf(i) !== -1 && [2].indexOf(j) !== -1) {
			makesound(bufferList[1], rnd(1,3)/3, 0.05, time(i+1), time(i));
		    }
			...
			makesound(bufferList[2], rnd(1,2)/3, 0.06, time(i+1), time(i));
		    }
