...
var gen = _.debounce(function(okgo) {
	...
	for (var h =  0; h < 3; h++) {
		...
		for (var i = 0; i < 8; ++i) {
		    ...
		    if ([0].indexOf(i) !== -1) {
			var voices  =  _.map(voices1, function(x){return x*Math.pow(2,rnd(-1,-1))});

			var o = 0.7;
			var p = rnd(0,3);
			var vol = 0.1;
			var v = new Voice(voices[p] * o);
			v.startx(time(i), 1.3,vol);
		    ...
		    if ([0,5].indexOf(i) !== -1) {
			makesound(bufferList[3], 1, 0.05, time(i+4.2), time(i+4));

			var speed = rnd(10,20)/10;
			var delta = (rnd(2,6)/10);
			var inst = 2;
			for (var hidx = 0; hidx < 6; hidx++) {
			    var t = time(i+2+hidx*delta );
			    makesound(bufferList[inst], speed-hidx/50, 0.05, t + 1, t, makefilter());
			}
		    }
